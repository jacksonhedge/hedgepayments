...
type Tester = { id: string; email: string; phone: string | null; first_name: string; last_name: string | null; age_bucket: string; state: string; platforms: string[]; verticals: string[]; status: string; sms_opt_in: boolean; email_opt_in: boolean; email_verified_at: string | null; referral_code: string | null; referral_source: string | null; signup_source: string | null; signup_path: string | null; attribution: Record<string, any> | null; payout_method: string | null; payout_handle: string | null; notes: string | null; created_at: string; research_assignments: { id: string; status: string; test_id: string }[]; research_messages: { id: string; channel: string; sent_at: string }[] }
              ...
              <tbody>
                {visible.map((t) => (
                  <tr key={t.id} className="border-t border-[#F0E8DD] align-top">
                    ...
                    <td className="p-2 text-xs" title={JSON.stringify(t.attribution || {}, null, 1)}>{t.signup_source || 'direct'}{t.referral_code && <div className="text-green-800">ref: <span className="font-mono">{t.referral_code}</span></div>}{t.referral_source && <div className="text-[#6B5D4F]">“{t.referral_source}”</div>}<div className="text-[#9a8b7a]">{t.signup_path || ''}{t.attribution?.device ? ` · ${t.attribution.device}` : ''}{t.attribution?.referrer ? ` · via ${t.attribution.referrer}` : ''}</div></td>
