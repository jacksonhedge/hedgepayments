...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await db.from('research_tests').insert({
    ...
    payout_cents: Math.max(0, Math.round(Number(b.payout_dollars || 0) * 100)),
...
export async function PATCH(req: NextRequest) {
  ...
  for (const k of ['title', 'description', 'instructions', 'status', 'starts_at', 'ends_at', 'platform_id']) if (k in b) patch[k] = b[k]
