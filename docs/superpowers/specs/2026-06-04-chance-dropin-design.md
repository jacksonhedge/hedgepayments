...
**Live market data (2026-06-04).** Markets are now fetched **live from Polymarket's public Gamma API** (`gamma-api.polymarket.com`, CORS-open → called directly from the browser, no backend) on open. Candidates are filtered to liquid (≥$250), open, resolving in 1h–21d markets, matched within `BAND` (±0.075) of the target probability, sorted closest-to-odds then most-liquid, capped at `SHOW_CAP` (12). Each row shows the real **expiration** (`fmtExpiry`: "ends in 6h" / "ends Sat" / "ends Jun 19") and appends the outcome for non-Yes/No markets. The **seed snapshot is the silent fallback** if the fetch fails or returns <4 markets. **Kalshi** has no CORS-open public endpoint (403), so it shows a "· soon" chip + "coming online" state until a backend proxy is added (the geo gate still routes US→Kalshi conceptually). The `api-base` seam remains for a hosted offers API later.
