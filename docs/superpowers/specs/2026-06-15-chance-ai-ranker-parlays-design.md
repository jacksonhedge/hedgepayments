...
- **`/offers/rank` public-endpoint auth + LLM budget (security follow-up).** The endpoint is a public embed (shopper's browser), so it can't use the server-to-server `x-hedge-key` scheme. Today it's protected by the global rate limiter + default-`off` (an unknown `merchantId` triggers no LLM call) + bounded inputs + no settings echo. Before a real provider is enabled in production, add a per-merchant **publishable key + origin allowlist + per-merchant LLM spend budget/quota** so a known paid-provider `merchantId` can't be hammered. Flagged by automated security review 2026-06-15.
